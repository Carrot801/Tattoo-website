  ...
  return (
    <nav className={`bg-black/50 backdrop-blur-sm p-1 shadow-lg fixed top-0 left-0 right-0 z-50 transform transition-transform duration-500  ${showNavbar ? "translate-y-0" : "-translate-y-full"}`}>
      ...
      <div className="container mx-auto flex items-center justify-end">
        <div className="flex flex-wrap items-center h-full space-x-4 sm:space-x-6">
          {['home', 'about-section', 'gallery-section', 'contact-section'].map(section => (
            ...
            >
              {section === 'home' ? 'Home' : section === 'about-section' ? 'About' : section === 'gallery-section' ? 'Gallery' : 'Contact'}
            </a>
          ))}
          <a href="#"><img className="w-5 h-5 sm:w-10 sm:h-10" src="https://img.icons8.com/fluency-systems-regular/48/FFFFFF/instagram-new.png" alt="instagram"/></a>
          <a href="#"><img className="w-5 h-5 sm:w-10 sm:h-10" src="https://img.icons8.com/fluency-systems-regular/48/FFFFFF/facebook-f.png" alt="facebook"/></a>
        </div>
      </div>
    </nav>
  );
